...
void effectManager::addEffect_png(string effectName, const char * imageName, float effectFPS, int buffer)
{
	gdipImage* img;
	vEffect vEffectBuffer;

	img = GDIPLUS->findImage(imageName);
	for (int i = 0; i < buffer; i++)
	{
		vEffectBuffer.push_back(new effect);
		vEffectBuffer[i]->init_png(img, effectFPS);
	}
...
void effectManager::play_png(string effectName, int x, int y)
{
	miEffect mIter;
	...
	for (mIter = _mEffect.begin(); mIter != _mEffect.end(); ++mIter)
	{
		...
		{
			if (mIter->second[i]->getIsRunning()) continue;
			mIter->second[i]->startEffect_png(x, y);
			return;
		}
	}
}
