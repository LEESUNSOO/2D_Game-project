...
HRESULT animation::init_png(int totalW, int totalH, int frameX, int frameY)
{
	//한프레임의 가로크기
	_frameWidth = frameX;
	...
	//총프레임의 갯수
	_frameNum = (totalW / frameX) * (totalH / frameY);

	//이미지 위치 세팅
	_frameList.clear();
	for (int i = 0; i < (totalH / frameY); i++)
	{
		for (int j = 0; j < (totalW / frameX); j++)
		{
			POINT framePos;
			framePos.x = j * _frameWidth;
			framePos.y = i * _frameHeight;
