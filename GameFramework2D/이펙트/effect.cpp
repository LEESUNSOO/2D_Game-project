...
HRESULT effect::init_png(gdipImage * effectImg, float effectFPS)
{
	//변수 초기화
	_effectPngImg = effectImg;
	_effectFPS = effectFPS;
	_isRunning = false;

	//이펙트 애니메이션 생성 및 초기화
	_effectAni = new animation;
	_effectAni->init_png(_effectPngImg->getWidth(), _effectPngImg->getHeight(), _effectPngImg->getFrameWidth(), _effectPngImg->getFrameHeight());
	_effectAni->setPlayFrame();
	_effectAni->setFPS(1);
...
void effect::render(void)
{
	if (_isRunning)
	{
		...
		if (_effectPngImg != NULL)
			_effectPngImg->aniRender(getMemDC(), _x, _y, _effectAni);
	}
}
...
void effect::startEffect_png(int x, int y)
{
