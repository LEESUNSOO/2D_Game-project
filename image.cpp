...
void image::render(HDC hdc, int destX, int destY, float sourX, float sourY, float sourWidth, float sourHeight)
{
	if (_isTrans) //배경색 없앨꺼냐?
	{
...
void image::stretchRender(HDC hdc, int destX, int destY, float sourX, float sourY, float sourWidth, float sourHeight, float scale)
{
	_stretchImage->width = _imageInfo->width * scale;
	_stretchImage->height = _imageInfo->height * scale;
