...
void imageManager::stretchRender(string strKey, HDC hdc, int destX, int destY, float sourX, float sourY, float sourWidth, float sourHeight, float scale)
{
	//이미지를 찾아서 그냥 이미지클래스의 함수로 렌더시키면 됨
	image* img = findImage(strKey);
	if (img) img->stretchRender(hdc, destX, destY, sourX, sourY, sourWidth, sourHeight, scale);
}
