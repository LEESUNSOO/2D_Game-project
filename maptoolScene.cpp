...
{
	//타일맵 이미지 초기화
	IMAGEMANAGER->addFrameImage("grass", "resource/maptool/grass_tiles.bmp", 640, 832, SAMPLETILEX, SAMPLETILEY);
...
{
	//타일맵 이미지 렌더
	IMAGEMANAGER->render("grass", getMemDC(), 660, 0);
	...
	for (int i = 0; i < TILEX * TILEY; i++)
	{
		...
		IMAGEMANAGER->frameRender("grass", getMemDC(), _tiles[i].rc.left, _tiles[i].rc.top, _tiles[i].terrainFrameX, _tiles[i].terrainFrameY);
	}
	//전체화면 왼쪽에 오브젝트를 그린다
	for (int i = 0; i < TILEX * TILEY; i++)
	{
		if (_tiles[i].obj == OBJECT_NONE) continue;
		IMAGEMANAGER->frameRender("grass", getMemDC(), _tiles[i].rc.left, _tiles[i].rc.top, _tiles[i].objFrameX, _tiles[i].objFrameY);
	}
	...
	TextOut(getMemDC(), _rcClear.left + 10, _rcClear.top + 10, "CLEAR", strlen("CLEAR"));
...
void maptoolScene::maptoolSetup(void)
{
	...
	_rcClear = RectMake(660 + 240, 400, 100, 50);
...
void maptoolScene::setMap(void)
{
	...
	for (int i = 0; i < TILEX * TILEY; i++)
	{
		if (PtInRect(&_tiles[i].rc, _ptMouse))
		{
			...
			{
				_tiles[i].objFrameX = 20;
				_tiles[i].objFrameY = 20;
				_tiles[i].obj = OBJECT_NONE;
				_tiles[i].terrain = TR_NONE;
...
void maptoolScene::clear()
{
	...
	{
		_tiles[i].obj = OBJECT_NONE;
		_tiles[i].objFrameX = 9;
		_tiles[i].objFrameY = 12;
	}
}
