...
HRESULT stage1_1_SceneT::init(void)
{
	...
	_map->init();
	_eMG->load(STAGE2); // ¸Ê·Îµå°¡ ¾Æ´Ï¶ó ¿¡³Ê¹Ì Á¤º¸ÀÌ´Ù!
	_eMG->init();
	_ui->init();
...
void stage1_1_SceneT::update(void)
{
	...
	CAMERAMANAGER->bgCameraUpdate("ÇÃ·¹ÀÌ¾îÄ«¸Þ¶ó", _player->getX(), _player->getY(), 1280*4 , 720);
	...
	if (_player->getX() < 10)
	{
		...
		SCENEMANAGER->loadScene("½ºÅ×ÀÌÁö1-1");
